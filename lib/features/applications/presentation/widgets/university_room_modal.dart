...
class UniversityRoomModal extends StatefulWidget {
  ...
  static void show(BuildContext context, StudentApplication app, {int initialTabIndex = 0}) {
...
class _UniversityRoomModalState extends State<UniversityRoomModal> {
  ...
  void initState() {
    ...
    _eventsController = UniversityEventsController(widget.application.universityId);
        ...
        children: [
          CircleAvatar(
            ...
            child: msg.senderAvatar == null 
                ? Text((msg.senderName ?? 'U').isNotEmpty ? (msg.senderName ?? 'U')[0] : 'U', style: const TextStyle(color: AppColors.vibrantLime, fontSize: 12))
                : null,
          ),
  ...
  Widget build(BuildContext context) {
    ...
    final selectedEvents = _selectedDay != null ? _getEventsForDay(_selectedDay!) : _getEventsForDay(_focusedDay);
          ...
          children: [
                ...
                children: [
                    CircleAvatar(
                      ...
                      child: Text(
                        ...
                        style: const TextStyle(color: AppColors.vibrantLime, fontWeight: FontWeight.bold),
                      ),
                    ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const Row(
                          children: [
                            Icon(Icons.hub_outlined, size: 12, color: Colors.white54),
                            SizedBox(width: 4),
                            Text(
                              'University Room',
            ...
            const TabBar(
              ...
              tabs: [
                ...
                Tab(icon: Icon(Icons.forum_outlined, size: 20), text: 'Discussion'),
                Tab(icon: Icon(Icons.announcement_outlined, size: 20), text: 'News'),
                Tab(icon: Icon(Icons.event_note_outlined, size: 20), text: 'Calendar'),
              ],
            ),
                ...
                children: [
                      ...
                      Expanded(
                        child: chatState.isLoading 
                            ...
                            : chatState.error != null
                                ? Center(child: Padding(padding: const EdgeInsets.all(16), child: Text(chatState.error!, style: const TextStyle(color: Colors.redAccent))))
                                : chatState.messages.isEmpty
                                    ? Center(child: Text('No messages yet. Start the conversation!', style: TextStyle(color: Colors.white.withOpacity(0.5))))
                                    : ListView.builder(
                                        reverse: true, // Optimizes large chat history scaling natively
                            ...
                            children: [
                              Expanded(
                                child: TextField(
                                  ...
                                  enabled: !chatState.isLoading && chatState.channelId != null,
                              ...
                              const SizedBox(width: 8),
                              CircleAvatar(
                                backgroundColor: (!chatState.isLoading && chatState.channelId != null) 
                                    ? AppColors.vibrantLime 
                                    : Colors.white24,
                                radius: 20,
                                child: IconButton(
                                  padding: EdgeInsets.zero,
                                  icon: const Icon(Icons.arrow_upward, color: AppColors.pureBlack, size: 20),
                                  onPressed: (!chatState.isLoading && chatState.channelId != null) 
                                      ? () {
                  ...
                  Center(
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.campaign_outlined, size: 48, color: Colors.white.withOpacity(0.1)),
                        const SizedBox(height: 16),
                        const Text('No active announcements.', style: TextStyle(color: Colors.white38)),
                      ],
                    ),
                  ),
                      ...
                      Padding(
                        ...
                        child: TableCalendar<UniversityEvent>(
                          ...
                          calendarStyle: CalendarStyle(
                            ...
                            selectedTextStyle: const TextStyle(color: AppColors.pureBlack, fontWeight: FontWeight.bold),
                            markersMaxCount: 3,
                          ...
                          : selectedEvents.isEmpty
                              ? Center(
                                  child: Column(
                                    mainAxisAlignment: MainAxisAlignment.center,
                                    children: [
                                      Icon(Icons.event_busy, size: 36, color: Colors.white.withOpacity(0.1)),
                                      const SizedBox(height: 12),
                                      const Text('No events to display on this date.', style: TextStyle(color: Colors.white38)),
                                    ],
                                  ),
                                )
                              : ListView.builder(
                                    ...
                                    return Container(
                                      ...
                                      child: ListTile(
                                        leading: Container(
                                          ...
                                          child: Icon(
                                            evt.eventType == 'deadline' ? Icons.timer : Icons.event, 
                                            color: AppColors.vibrantLime, 
                                            size: 20
                                          ),
                                        ),
                                        title: Text(evt.title, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)),
                                        subtitle: Column(
                                          crossAxisAlignment: CrossAxisAlignment.start,
                                          children: [
                                            if (evt.description != null && evt.description!.isNotEmpty) ...[
                                              const SizedBox(height: 4),
                                              Text(evt.description!, style: const TextStyle(color: Colors.white70, fontSize: 12)),
                                            ],
                                            const SizedBox(height: 6),
                                            Text(DateFormat('hh:mm a').format(evt.eventDate), style: const TextStyle(color: Colors.white38, fontSize: 12)),
                                          ],
                                        ),
                                      ),
                                    );
