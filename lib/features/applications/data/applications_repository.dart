...
final suggestedUniversitiesProvider = FutureProvider<List<University>>((ref) async {
  ...
  try {
    ...
    debugPrint('[Suggestions] student_suggestions query for ${user.id} returned ${(data as List).length} rows');
    ...
    if (suggestions.isNotEmpty) {
      debugPrint('[Suggestions] returning ${suggestions.length} CRM suggestions');
      return suggestions;
    }
  } catch (e) {
    if (e is PostgrestException && e.code == 'PGRST205') {
       debugPrint('[Suggestions] student_suggestions table missing, falling back.');
    } else {
  ...
  try {
    ...
    final fallbackData = await client
        ...
        .eq('is_partner', true)
        .limit(5);
    debugPrint('[Suggestions] Fallback partner universities returned ${(fallbackData as List).length} rows');
    if ((fallbackData as List).isEmpty) {
      debugPrint('[Suggestions] WARNING: Fallback returned 0 universities. Ensure is_partner=true is set for some universities.');
    }

    return fallbackData.map<University>((u) => University(
      id: u['id'] as String,
      name: u['name_en'] as String? ?? u['name_uz'] as String? ?? 'Unknown',
      location: u['city_en'] as String? ?? '',
      isPartner: u['is_partner'] as bool? ?? false,
...
Future<void> submitSelectedUniversities(List<String> universityIds) async {
  ...
  final inserts = universityIds.map((uId) => {
    'student_id': user.id,
    'university_id': uId,
  ...
  try {
    ...
    debugPrint('[ApplicationsRepository] Failed to clear remaining suggestions: $e');
    // We swallow this error because the application was successfully submitted
  }
}

final applicationsProvider =
    FutureProvider<List<StudentApplication>>((ref) async {
  // Watch the provider directly (sync) to avoid async stream .future caching bugs
  final authState = ref.watch(authStateProvider);
  final user = authState.value?.session?.user ?? Supabase.instance.client.auth.currentUser;
  if (user == null) {
  ...
  try {
    ...
    debugPrint('[Applications] Query for student ${user.id} returned ${(data as List).length} applications');
    ...
    debugPrint('[ApplicationsRepository] Failed to fetch applications: $e\n$st');
    throw e;
  }
});
