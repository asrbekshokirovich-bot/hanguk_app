...
class VapiWebCall implements VapiCall {
  ...
  static Future<VapiWebCall> create(
    ...
    final jsAssistantValue = assistantValue is Map
        ? assistantValue.jsify() as JSAny
        : (assistantValue as String).toJS as JSAny;
    ...
    final Object? dartified = jsCallData.dartify();
    final callData = dartified is Map ? Map<String, dynamic>.from(dartified) : <String, dynamic>{};

    final id = callData['id'] as String? ?? 'web_call_id';
    final orgId = callData['orgId'] as String? ?? '';
    ...
    final type = callData['type'] as String? ?? 'webCall';
    
    final monitorTmp = callData['monitor'] is Map ? Map<String, dynamic>.from(callData['monitor']) : <String, dynamic>{};
    final monitor = VapiCallMonitor.fromJson(monitorTmp);
    
    final transportTmp = callData['transport'] is Map ? Map<String, dynamic>.from(callData['transport']) : <String, dynamic>{};
    final transport = VapiCallTransport.fromJson(transportTmp);
    
    final webCallUrl = callData['webCallUrl'] as String? ?? '';
    final assistantId = callData['assistantId'] as String? ?? '';
    final overridesRaw = callData['assistantOverrides'];
    final assistantOverrides = overridesRaw is Map
